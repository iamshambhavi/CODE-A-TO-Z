...
mongoose.connect("mongodb://localhost:27017/codeatoz",{ useNewUrlParser: true,useUnifiedTopology:true});
...
var upload = multer({ 
    storage:storage
 }).fields([{name: 'profilePhoto'},{name: 'coverPhoto'}]);
...
//handle sign up logic
	app.post("/register",upload, function(req, res){
			...
			console.log(req.body.imgjajaja);
			var newUser = new User({first: req.body.first, last: req.body.last, email: req.body.email, username: req.body.username, gender: req.body.gender, status: req.body.status, institution: req.body.institution, country: req.body.country, profilePhoto: req.files['profilePhoto'][0].filename, coverPhoto: req.files['coverPhoto'][0].filename});
		console.log(newUser);
